...
const ManageByLaw = () => {
  ...
  useEffect(() => {
    fetch(`${apiUrl}/by-law/all`)
      ...
      .then((data) => {
        if (data.success) {
          setValue("title", data.byLaw.title);
          setValue("description", data.byLaw.description);
        } else {
          alert(data.msg);
        }
      })
  ...
  const onSubmit = (data) => {
    ...
    fetch(`${apiUrl}/by-law/update`, {
      ...
      body: JSON.stringify(byLawContent),
    })
      .then((res) => res.json())
      .then((data) => {
        setIsLoading(false);
